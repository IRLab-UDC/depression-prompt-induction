...
TEST_DATA_PATH = Path(args.data_dir) / f"{args.split}.jsonl"
TRAIN_DATA_PATH = Path(args.data_dir) / "train.jsonl"
OUTPUT_PATH = f"runs/cross_domain/{MODEL.replace('/', '_')}_{args.split}_{N_SHOTS}shot_icl_psysym.json"
...
for symptom in symptoms_info:
    ...
    for i in range(len(sentences)):
        ...
        predictions_with_sentences.append({
            ...
            "diseases": test_samples[i]["diseases"],
            "kb_symptoms": test_samples[i]["kb_symptoms"],
            "is_control": test_samples[i]["is_control"]
        })
