...
def evaluate(results):
    ...
    per_disease = compute_per_disease_metrics(results)
...
def compute_per_disease_metrics(results):
    disease_data = defaultdict(lambda: defaultdict(lambda: {"predictions": [], "ground_truth": []}))
    ...
    for r in results:
        ...
        for detail in details:
            diseases = detail.get("diseases", [])
            pred = detail["prediction"]
            gt = detail["ground_truth"]

            for disease in diseases:
                disease_data[disease][symptom]["predictions"].append(pred)
                disease_data[disease][symptom]["ground_truth"].append(gt)

    per_disease_metrics = {}

    for disease, symptom_dict in disease_data.items():
        all_preds = []
        all_gt = []
        per_symptom = {}

        for symptom, data in symptom_dict.items():
            preds = data["predictions"]
            gt = data["ground_truth"]
            all_preds.extend(preds)
            all_gt.extend(gt)
            per_symptom[symptom] = compute_binary_metrics(gt, preds)

        overall = compute_binary_metrics(all_gt, all_preds)

        symptom_f1s = [m["f1"] for m in per_symptom.values()]
        macro_f1 = float(np.mean(symptom_f1s)) if symptom_f1s else 0.0
