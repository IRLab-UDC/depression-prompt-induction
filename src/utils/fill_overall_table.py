...
print("\\begin{table}")
print("    \\centering")
print("    \\caption{Weighted precision, recall, and F1-score for multilabel classification across different model sizes and inference strategies on the BDI-Sen test set. \\textbf{Bold}/\\underline{underlined} indicate best/second-best per row (strategy comparison). $^\\dagger$/$^\\ddagger$ indicate best/second-best per column (model comparison).}")
print("    \\label{tab:bdi_sen_overall}")
print("    \\begin{tabular}{llcccc}")
print("    \\toprule")
print("    \\textbf{Metric} & \\textbf{Model} & \\textbf{ZS} & \\textbf{ICL} & \\textbf{FT} & \\textbf{SI} \\\\")
print("    \\midrule")

for metric in ["precision", "recall", "f1"]:
    metric_label = metric.capitalize() if metric != "f1" else "F1"
    print(f"    \\multirow{{6}}{{*}}{{{metric_label}}}")

    col_best = {}
    for m in ["zs", "icl", "sft", "rcl"]:
        col_values = []
        for model_id in models:
